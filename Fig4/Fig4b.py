...
Na, Nb, Nc = int(1.5*na),int(1.5*nb),int(1.5*nc);

wave = np.zeros([2,2*Na+1,2*Nb+1,2*Nc+1]);
M = np.zeros([2,2,3]);
rec = 0;
...
rmn = [[np.linalg.norm(M[u,v]/np.linalg.norm(wave[u])/np.linalg.norm(wave[v])) for v in range(2)] for u in range(2)];
print(rmn)

mH = 1.0078;
mLi = 6.9410;
M_sphere = n*d**3/4**3*(mH+mLi)*4*1.66054*10**-27;
dE = (kappa[0]**2-kappa[1]**2)*24.046*10**3
alpha_s = 1/np.linalg.norm(wave[0]);
alpha_p = 1/np.linalg.norm(wave[1]);

out = {'rmn':np.linalg.norm(rmn[0][1]),'dE':dE,
