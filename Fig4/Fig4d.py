...
Omega = [];
for R in [270+20*i for i in range(10)]:
    ...
    wave = np.zeros([4,2*Na+1,2*Nb+1,2*Nc+1]);
    M = np.zeros([4,4,3]);
    rec = 0;
    ...
    Omega.append(q*E/M_sphere*mn/hbar*rmn[0][1]*10**-10*10**-6/2/np.pi);
    
    import json;   
    with open('omega_r.json','w') as file:
